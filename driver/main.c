...
static inline void **ptr_table_find(struct ptr_table *table, void *ptr)
{
	...
	for(p = startp; p != endp; ++p)
	{
		if(!*p || *p == ptr)
			return p;
		if(!firstfree && PTR_TABLE_IS_FREE(*p))
			firstfree = p;
		++p;
	}
...
static inline bool ptr_table_add(struct ptr_table *table, void *ptr)
{
	...
	if(!*p)
		++table->used_slots; // only increment used_slots if the slot is really free, not "free again".
	*p = ptr;
	++table->size;
...
static struct ptr_table *ptr_table_create(int new_capacity, struct ptr_table *old_table)
{
	...
	if(old_table && new_capacity < old_table->size)
		return NULL;
...
static int add_excluded_pid_ns(struct pid_namespace *pid_ns)
{
	...
	ptr_table_add(table, pid_ns);
	if(table->used_slots * 2 < ptr_table_capacity(table))
		goto exit;
	new_capacity = (table->size << 2) + 3;
	if(new_capacity < EXCLUDED_PID_NS_TABLE_INITIAL_CAPACITY)
		new_capacity = EXCLUDED_PID_NS_TABLE_INITIAL_CAPACITY;
...
static int ppm_open(struct inode *inode, struct file *filp)
{
	...
	*/
	...
	// bitmap_fill(consumer->syscalls_mask, SYSCALL_TABLE_SIZE); /* Enable all syscall to be passed to userspace */
	/* End StackRox section */
	...
	{
		bitmap_zero(consumer->syscalls_mask, SYSCALL_TABLE_SIZE); /* Zero out so that no syscall squeaks past us */
					   ...
					   struct event_data_t *event_datap)
{
	...
	list_for_each_entry_rcu(consumer, &g_consumer_list, node)
	{
		...
		if(!test_bit(event_type, consumer->syscalls_mask))
			continue;
		/* End StackRox section */
