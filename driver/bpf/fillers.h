...
/* StackRox: Only add cpuacct if cpu or cpu_group cgroup path not included */
#if !IS_ENABLED(CONFIG_CGROUP_SCHED)
...
/* StackRox: Remove io/blkio to reduce the number of cgroup paths */
#if 0
...
FILLER(proc_startupdate, true)
{
	...
	if (args_len) {
		 ...
		 * exe
		 */
		res = bpf_val_to_ring_type_mem(data, (unsigned long)&empty, PT_CHARBUF, KERNEL);
		if (res != PPM_SUCCESS)
			return res;
		 ...
		 * Args
		 */
		res = bpf_val_to_ring_type_mem(data, 0, PT_BYTEBUF, KERNEL);
		if (res != PPM_SUCCESS)
			return res;
	}
...
FILLER(sys_quotactl_x, true)
{
	...
	if (cmd == PPM_Q_QUOTAON) {
		...
		res = bpf_val_to_ring_type_mem(data, (unsigned long)empty, PT_CHARBUF, USER);
		if (res != PPM_SUCCESS)
			return res;
	}
...
FILLER(sched_prog_exec, false)
{
	...
	{
		/* Parameter 2: exe (type: PT_CHARBUF) */
		res = bpf_val_to_ring_type_mem(data, 0, PT_CHARBUF, KERNEL);
		if(res != PPM_SUCCESS)
		{
			return res;
		}
